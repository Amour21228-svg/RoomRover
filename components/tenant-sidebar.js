class CustomTenantSidebar extends HTMLElement {
    connectedCallback() {
        // Détecter le contexte
        const isInPages = window.location.pathname.includes('/pages/');
        const basePath = isInPages ? '../pages' : './pages';
        const dashboardPath = isInPages ? '../tenant.html' : './tenant.html';
        const logoutPath = basePath + '/logout.html';
        
        // Get user info from auth-client
        const user = window.auth && window.auth.getUser ? window.auth.getUser() : null;
        const userDisplayName = user ? `${user.firstName} ${user.lastName}` : 'Locataire';
        ...
        this.innerHTML = `
            ...
            <aside class="w-64 bg-white dark:bg-gray-800 shadow-md h-screen fixed hidden md:block">
                ...
                <nav class="p-4 space-y-1">
                    <a href="${dashboardPath}" class="sidebar-link active flex items-center space-x-3 px-3 py-2 rounded-lg font-medium text-gray-700 dark:text-gray-300">
                        <i data-feather="home" class="w-5 h-5"></i>
                        <span>Tableau de bord</span>
                    </a>
                    <a href="${basePath}/properties.html" class="sidebar-link flex items-center space-x-3 px-3 py-2 rounded-lg font-medium text-gray-700 dark:text-gray-300">
                        <i data-feather="key" class="w-5 h-5"></i>
                        <span>Ma location</span>
                    </a>
                    <a href="${basePath}/receipt.html" class="sidebar-link flex items-center space-x-3 px-3 py-2 rounded-lg font-medium text-gray-700 dark:text-gray-300">
...
customElements.define('custom-tenant-sidebar', CustomTenantSidebar);